...
namespace Service
{
    public class TeamMemberService : BaseDatabaseService<IrisContext>, ITeamMemberService
    {
        private readonly IConfiguration _configuration;
        private readonly IFileService _fileService;
        private readonly string? _uploadPath;

        public TeamMemberService(IrisContext context, IConfiguration configuration, IFileService fileService) : base(context)
        {
        ...
        public async Task<BaseResponse<List<TeamMemberDTO>>> GetAllAsync(BaseRequest request)
        {
        ...
        public async Task<BaseResponse<TeamMemberDTO?>> GetByIdAsync(int id)
        {
            return await HandleActionAsync(async () =>
            {
                var dbEntity = await _context.TeamMembers
                    .AsNoTracking()
        ...
        public async Task<BaseResponse<Task>> AddAsync(int userId, TeamMemberAddDTO request)
        {
            return await HandleVoidActionAsync(async () =>
            {
                var dbEntity = new TeamMember
                {
                ...
                await _context.TeamMembers.AddAsync(dbEntity);
                await _context.SaveChangesAsync(userId);
            });
        }

        public async Task<BaseResponse<Task>> UpdateAsync(int userId, TeamMemberUpdateDTO request)
        {
            return await HandleVoidActionAsync(async () =>
            {
                var (doesNotExist, dbEntity) = await DoesNotExistAsync<TeamMember>(x => x.Id == request.Id);
                ...
                _context.TeamMembers.Update(dbEntity);
                await _context.SaveChangesAsync(userId);
            });
        }
        ...
        public async Task<BaseResponse<Task>> DeleteAsync(int userId, int id)
        {
            return await HandleVoidActionAsync(async () =>
            {
                var (doesNotExist, dbEntity) = await DoesNotExistAsync<TeamMember>(x => x.Id == id);
